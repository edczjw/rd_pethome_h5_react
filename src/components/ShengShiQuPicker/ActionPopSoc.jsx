...
class ActionPopSoc extends Component {
    constructor(props){
        super(props)
        this.state=({
            currentIndex:4,
    ...
    handleSelect(index,value){ 
        ...
        this.props.parent.getSocMsg(this, index,value)
        //关闭弹窗
        setTimeout(()=>{
    ...
    render() { 
        var li = this.props.myList.map((item,index)=>{
            return <li onClick={()=>this.handleSelect(index+4,item.value)} key={index} className={index===this.state.currentIndex-4? 'active':null}>{item.value}</li>
        })
        return (
            <div>
            {
